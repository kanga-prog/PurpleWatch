...
## 18. Collection Linux T1119 — 24 septembre 2026

Scénario Caldera `PW-T1119-Linux-AutomatedCollection` : sélection `find` de fichiers `.txt` factices dans `/var/lib/purplewatch/t1119/input` et copie `cp` dans `/var/lib/purplewatch/t1119/collected`. Deux essais à 09:04:20 et 09:12:41 UTC+2 ont échoué car Sandcat lancé via systemd a `PrivateTmp=yes` : le `/tmp` du shell n'était pas visible du service. L'isolation a été conservée et les données de test déplacées sous `/var/lib/purplewatch`.

Baseline Caldera `success` à 09:34:05 UTC+2 : les deux `.txt` factices ont été copiés, le fichier `ignore.bin` exclu ; auditd `find` événement 13906, PID 14875/PPID 14874 (shell Sandcat), puis `cp` événement `1790235269.548:13907`, PID 14876/PPID 14875, arguments source/destination explicites. La règle `100109` (MITRE T1119, niveau 6) cible `/usr/bin/cp`, la clé audit et les arguments vers le répertoire factice dans l'événement SYSCALL+EXECVE regroupé. Tests locaux : positif `100109`, négatif autre destination `80700`.

Retest Caldera `success` à 09:59:22 UTC+2 (PID affiché 14993) ; Wazuh montre une alerte `100109` à 09:59:51.355 UTC+2 sur `PW-LINUX-01` : **PASS de détection E2E pour la tactique Collection**. Écart des heures affichées 29,355 s, non assimilé à une latence E2E exacte. ID de l'alerte, événement auditd et confirmation des fichiers de ce retest restent à relever.

Complément T1059.004 : alerte Wazuh `1790210569.50008`, auditd `id=5850`, `pid=5808`, `ppid=5807` égal au PID Caldera du retest.

## 19. Defense Evasion T1036.005 et Credential Access T1552.001 — 24 septembre 2026

T1036.005 : copie inoffensive de `/usr/bin/true` nommée `/var/lib/purplewatch/t1036/systemd`. Baseline Caldera 10:12:25 UTC+2, PID 15121 ; auditd `1790237578.760:14175`, PID 15122, PPID 15121, `exe=/var/lib/purplewatch/t1036/systemd`, clé `purplewatch_execve`. Règle Wazuh `100110`, niveau 6, chemin isolé exact. `wazuh-logtest` positif sur baseline, négatif sur vrai chemin `/usr/lib/systemd/systemd` (80700). Retest Caldera `success` 10:40:28 UTC+2, PID 15233 ; Wazuh `100110` à 10:41:21.650 : **PASS E2E Defense Evasion**. Écart affiché 53,650 s ; aucune latence exacte inférée.

T1552.001 : fichier leurre `/var/lib/purplewatch/t1552/decoy.env`, contenu purement fictif `PW_TEST_ONLY=NOT_A_REAL_CREDENTIAL`, mode 600 ; règle auditd de lecture `-w ... -p r -k purplewatch_decoy_read`. Baseline Caldera 10:49:51 UTC+2, PID 15288 ; auditd `1790239834.126:14400`, `openat` lecture réussie `O_RDONLY`, PID 15289, PPID 15288, `exe=/usr/bin/cat`, `PATH` du leurre. Règle Wazuh `100111`, niveau 6, sur `audit.syscall=257`, succès, exécutable, clé et `audit.file.name`. Test positif `100111`, négatif sur `other.env` (80700). Retest Caldera `success` 11:00:55 UTC+2, PID 15340 ; alerte Wazuh `100111` 11:01:03.802 : **PASS E2E Credential Access**. Écart affiché 8,802 s ; latence exacte encore indéterminée.

Ainsi les quatre tactiques supplémentaires demandées (Execution, Collection, Defense Evasion, Credential Access) ont une détection E2E observée dans le laboratoire. Leurs identifiants d'alertes et événements des derniers retests restent à extraire, puis les adversaries combinés, métriques de répétitions et corrélation à valider.

## 20. Séquence de reconnaissance Linux — 24 septembre 2026

Adversary `PW-LINUX-Recon-Sequence` : abilities T1082, T1057, T1016 dans cet ordre sur `PW-LINUX-01`. Caldera affiche `success` à 11:24:23 (PID 15474), 11:24:28 (PID 15485) et 11:25:08 (PID 15487), UTC+2. Wazuh agent `002` : `100104` 11:24:25.971, ID `1790241865.609283` ; `100105` 11:25:05.979, ID `1790241905.611203` ; `100106` 11:25:58.024, ID `1790241958.613081`. Les alertes distinctes et ordonnées couvrent 92,053 s (< 300 s) sur le même agent. Une règle de corrélation déterministe `purplewatch_correlate.py` a passé les cas positifs, autre agent, ordre erroné et hors fenêtre en local ; exécution sur les alertes réelles du manager et intégration à Wazuh à confirmer.

## 21. Mesures et scénario à plusieurs tactiques — 24 septembre 2026

Corrélation lancée manuellement sur les vraies alertes de l'agent `002` : IDs `1790241865.609283`, `1790241905.611203`, `1790241958.613081`, fenêtre de 92,053 s. Elle ne génère pas encore automatiquement une alerte Wazuh.

Cinq opérations Caldera de reconnaissance, trois abilities réussies chacune, ont donné 15 alertes attendues sur 15. Latence entre l'événement auditd horodaté dans `full_log` et l'alerte Wazuh : **P50 0,874 s ; P95 1,178 s** (rang supérieur ; n=15). Résultat exploratoire ; aucune latence clic Caldera → alerte ou affichage Dashboard ne peut en être inférée. L'opération de 11:49:10 UTC+2, absente du premier tableau copié, a été confirmée par l'utilisateur.

Adversary `PW-LINUX-MultiTactic-Sequence` : T1059.004, T1082, T1057, T1016, T1087.001, T1552.001, T1119 et T1036.005 sur `PW-LINUX-01`. Le 24 septembre, de 12:24:00 à 12:29:55 UTC+2, **8/8** abilities Caldera `success` ; Wazuh agent `002` affiche les **8/8** alertes correspondantes dans le même ordre : `100108` 12:24:24.394, `100104` 12:25:16.391, `100105` 12:26:08.447, `100106` 12:26:46.408, `100107` 12:27:48.481, `100111` 12:28:52.454, `100109` 12:29:52.404 et `100110` 12:30:48.464 UTC+2. Cinq tactiques couvertes : Execution, Discovery, Credential Access, Collection et Defense Evasion. Les fichiers et identifiants utilisés sont fictifs. IDs d'alertes et chaînage PID/PPID de cette opération encore à extraire.

### Validation de l'alerte automatique 100112

Sur `PW-WAZUH-01`, le service `purplewatch-recon.service` suit les nouvelles lignes de `alerts.json`, le script `/usr/local/lib/purplewatch/recon_live.py` émet une ligne JSON par séquence `100104` → `100105` → `100106` sur un même agent en moins de 300 secondes ; le manager surveille `/var/ossec/logs/purplewatch-recon.jsonl` et sa règle `100112` de niveau 10 produit l'alerte. Le scénario mult tactiques répété à 13:03–13:08 UTC+2 donne **8/8** liens Caldera `success`, huit alertes individuelles et une alerte automatique `100112` horodatée `2026-09-24T11:06:04.374+0000`, ID `1790247964.708527`. Les trois alertes source sont `1790247877.701199`, `1790247930.704662`, `1790247963.706540` ; leur durée est **86,045 s**. L'alerte `100112` a `agent.id=000` (manager qui ingère le fichier) et `data.pw_source_agent_id=002` (endpoint Linux d'origine). Intervalle dernière alerte source → alerte de corrélation : **1,306 s** ; cela n'est pas une latence globale Caldera → Wazuh. Limite opérationnelle : le suivi démarre à la fin du fichier au lancement du service ; les alertes survenues pendant son arrêt ne sont pas rejouées automatiquement.
